...
class VendorController extends Controller
{
    public function vendorList(Request $request)
    {
        try {
            ...
            $users = $users->latest()->role('SAFARI_OPERATOR')->paginate($perPage)->withQueryString();

            return Inertia::render('Admin/vendor/List', ['users' => $users]);
        } catch (\Exception $e) {
    ...
    public function createVendor(Request $request)
    {
        ...
        return Inertia::render('Admin/vendor/CreateEdit');
    }
    ...
    public function editVendor(Request $request, $id)
    {
        ...
        if (request()->isMethod('post')) {
            $credentials = $request->validate([
                ...
                'dob' => 'required',
                'status' => 'required',
                'license' => $request->hasFile('license') ? 'mimes:pdf' : '',
        ...
        return Inertia::render('Admin/vendor/CreateEdit', compact('user'));
    }
    ...
    public function changeVendorStatus(Request $request)
    {
        try {
            ...
            $user->save();
            session()->flash('success', 'Vendor status successfully changed');
            return back();
        } catch (\Exception $e) {
            Log::error(" :: EXCEPTION :: " . $e->getMessage() . "\n" . $e->getTraceAsString());
            abort(500);
    ...
    public function vendorApproval(Request $request, $id)
    {
        $vendor = User::with('country')->where('id', $id)->first();
        $vendor->main_destination = $vendor->main_destination ? json_decode($vendor->main_destination) : [];

        if (request()->isMethod('post')) {
            $credentials = $request->validate([
                'is_approved' => ['required', 'not_in:' . $vendor->is_approved],
            ], [
                'is_approved.required' => 'Please select approval status.',
                'is_approved.in' => 'Please select a valid approval status.',
                'is_approved.not_in' => 'The approval status is already set to this value.',
            ]);
            ...
            try {
                ...
                if ($vendor->is_approved == 1) {
                    if (filter_var($vendor->email, FILTER_VALIDATE_EMAIL)) {
                        Mail::to($vendor->email)->send(new AdminApprovalMail($vendor));
                    }
                }
                session()->flash('success', 'Approval status successfully changed');
                return redirect()->route('admin.vendors');
            } catch (\Exception $e) {
                Log::error(" :: EXCEPTION :: " . $e->getMessage() . "\n" . $e->getTraceAsString());
                abort(500);
        ...
        return Inertia::render('Admin/vendor/VendorApproval', compact('vendor'));
    }
}
