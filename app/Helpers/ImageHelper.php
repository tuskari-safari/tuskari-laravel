...
class ImageHelper
{
    ...
    public static function customSaveDuplicateImage($fileOrPath, $folder, $duplicateSafariId)
    {
        // Define destination folder
        $destinationPath = $folder;

        if ($fileOrPath instanceof \Illuminate\Http\UploadedFile) {
            $fileName = uniqid() . time() . rand(10, 1000000) . '_' . $duplicateSafariId . '.' . $fileOrPath->getClientOriginalExtension();
