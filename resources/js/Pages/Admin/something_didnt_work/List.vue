<template>
    <div>
        <Head title="Something Didn't Work Reports" />
        <div class="kt-portlet kt-portlet--mobile">
            <div class="kt-portlet__body">
                        ...
                        <div class="col-sm-12">
                            <table
                                    ...
                                    <tr class="filter">
                                        ...
                                        <th>
                                            <select class="form-control form-control-sm form-filter kt-input"
                                                v-model="form.resolve" title="Select" data-col-index="2">
                                ...
                                <tbody v-auto-animate>
                                    <template v-for="issue in issues?.data" :key="issue?.id">
                                        <tr role="row" class="odd">
                                             <td class="align-center">{{issue.user?.full_name ?? 'NA'}}</td>
                                             <td class="align-center">{{issue.issue_description ? issue.issue_description.substring(0, 50) + '...' : '---'}}</td>
                                            <td class="align-center">
                                                <span @click="changeStatus(issue.id)" style="cursor: pointer"
                                                    class="kt-badge kt-badge--inline kt-badge--pill cursor-pointer"
                                                    :class="issue.resolve == 1 ? 'kt-badge--success' : 'kt-badge--warning'">{{
                                                        issue.resolve == 1 ?
                                            ...
                                            <td nowrap="" class="align-center">
                                                <span class="dropdown">
                                                    ...
                                                    <div class="dropdown-menu dropdown-menu-right">
                                                        <Link class="dropdown-item" :href="route('admin.something-did-not-work.view', issue.id)"><i class="la la-eye"></i> View</Link>
                                                    </div>
                                                </span>
                                            </td>
                                        </tr>
                                    ...
                                    <tr role="row" v-if="issues?.data?.length == 0" class="odd text-center">
                ...
                <div class="row" v-if="issues?.data?.length != 0">
                    <div class="col-sm-12 col-md-5">
                        <div class="dataTables_info" id="kt_table_1_info" role="status" aria-live="polite">
                            Showing {{ issues.from }} to {{ issues.to }} of {{ issues.total }} entries
                        </div>
                    </div>
                    <div class="col-sm-12 col-md-7">
                        <div class="float-right">
                            <Bootstrap4Pagination :data="issues" :limit="2"
...
const resetSearch = () => {
    router.visit(route("admin.something-did-not-work.index"), {
        method: "get",
    });
};
